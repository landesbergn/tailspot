...
- **Replay analyzer v0.** `ReplayAnalyzer` reads a recorded `.jsonl` (or an in-memory `[ReplayEvent]`) and runs each tick back through the same annotation + visibility + lock-on logic the live app uses, emitting one `ReplayTickReport` per tick (per-aircraft bearing/elevation/slant + visibility flag + screen projection, plus the closest-to-center icao24 and the lock-on engine state after the tick). The annotation logic itself lives on `ObservedAircraft.annotate(_:observer:now:)` — both the live `ADSBManager.reAnnotate` and the analyzer call it, so engine tweaks land in both paths simultaneously. Pure-Swift, MainActor; no UI yet (Pending §9 #3 follow-up).
- **109 unit tests** in `TailspotTests/` covering geometry, OpenSky decoding, annotation, sort, error handling, extrapolation, visibility predicate, screen projection, aircraft-metadata decoding, MetadataCache LRU+miss-as-hit semantics, ADSBManager metadata-cache-and-fallback, SwiftData Catch persistence (including the operatorName default), LockOnEngine state transitions (idle/acquiring/locked/sticky/**forceLock**), HangarGrouping (both modes, fallbacks, sort order, empty input, whitespace folding), the replay format (JSONL round-trip, partial-line tolerance, recorder lifecycle, Aircraft→AircraftSnapshot conversion), the replay analyzer (empty input, session-start header, GPS-less tick skips annotation, on-ground drop, visibility filter, lock acquires → locks → goes sticky across multi-tick sequences, file-based analyze round-trip), and `closestTargetIcao24` (center-default, empty zone returns nil, tap point picks nearest plane, empty-sky tap returns nil, narrow FOV/zoom pushes off-axis planes out of zone).

**Deliberately not yet built:** Hangar dedupe + delete (deferred from v0), replay-analyzer UI / CLI (analyzer ships but feeding it a real recording is currently a `let report = try ReplayAnalyzer().analyze(fileURL:)` away — no human-readable summary or diff-against-baseline yet), backend, ARKit drift correction, achievements/scoring, visual confirmation (CV/ML on the camera feed), origin/destination route info, device-side `os.Logger` capture (only system-emitted lines reach `bin/log-tail` today; see PLAN.md §9 #2). See PLAN.md §9 for the prioritized backlog. Don't try to "fix" what isn't built.
...
`SensorSnapshot.zoomFactor` is `Double?` (optional) for back-compat — files recorded before camera zoom shipped don't have the field; the analyzer treats nil as 1.0. **Tap-to-ID events are not yet captured**, so the analyzer's lock-on state will diverge from live behavior whenever the user tap-pinned a plane during a session. Adding a `.tapPin(icao24)` event to `ReplayEvent` is a future format addition; PLAN §9 #4 covers it.
...
PLAN.md §9 is the authoritative backlog. As of 2026-05-17, **Hangar v0, the replay recorder, the replay analyzer, and camera zoom + tap-to-ID have all shipped** (see the "Hangar collection," "Replay recorder," "Replay analyzer," and "Camera zoom + tap-to-ID" patterns above). Top of the queue now:
...
2. **Replay analyzer summary / CLI / debug view** (PLAN §9 #2) — analyzer ships structured `ReplayReport`s, but no human-readable formatter or `.jsonl` consumer exists yet. Cheap follow-up; becomes interesting now that Noah has recorded sessions.
3. **Hangar v1 polish** (PLAN §9 #3): dedupe, swipe-to-delete with confirm, illustrated cards. Defer until catch volume exists.
4. **Planespotters photo API integration** (PLAN §9 #4): real aircraft photos by reg/hex, replaces the commissioned-illustrated-cards plan if TOS allow.
5. **Visual confirmation** (Vision + COCO airplane class; PLAN §9 #5). Most invasive; tackle once analyzer summary exists so accuracy gains are measurable.

Lower priority: achievements/scoring (#6), OpenSky secret rotation (#7, demoted per Noah).
