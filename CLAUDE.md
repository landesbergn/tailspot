...
PLAN.md §9 is the authoritative backlog. As of 2026-05-18, **Hangar v0, the replay recorder, the replay analyzer (+ describe formatter + in-app loader), and camera zoom + tap-to-ID have all shipped**. Top of the queue now:
...
3. **Capture tap-pin events in replay** (PLAN §9 #3) — analyzer's lock-on state diverges from live when tap-to-ID was used. Adding `.tapPin(icao24)` to `ReplayEvent` closes the gap.
4. **Planespotters photo API integration** (PLAN §9 #4): real aircraft photos by reg/hex.
5. **Visual confirmation** (Vision + COCO airplane class; PLAN §9 #5).
