...
- **v1 authenticity gate (U4–U7) — the indoor-catch fix:** `SkyCheck` decides "pointed at open sky?" from frame structure + colour (night-aware; never brightness) corroborated by GPS accuracy, and **fails open** (only a confident `.notSky` blocks). Wired into the catch path **shadow-mode first** (`outdoor_gate_shadow`, never blocks), with flag-gated enforcement (`catch_blocked_outdoors` + a friendly "head outside" nudge) defaulting **OFF** until the field gate passes. A debug-overlay "Sky gate" row flips shadow↔enforce on-device. Offline validator + protocol at `tools/authenticity-gate/`.
