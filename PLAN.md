...
- **Catch-confirmation telemetry (U1–U2):** `catch_performed` (+ `is_duplicate`) and `catch_deleted` events; a subtle reveal-moment "is this right?" confirm/deny → `catch_confirmed`/`catch_denied` + an additive `Catch.confirmed` flag (nil = unanswered). The north-star (catch-confirmation-rate) is now a computable PostHog funnel — it previously had no negative signal. Logic in a pure `CatchTelemetry` helper; multi-catch confirm deferred (single-catch path only).
- **v1 authenticity gate (U4–U7) — the indoor-catch fix:** `SkyCheck` decides "pointed at open sky?" from frame structure + colour (night-aware; never brightness); the camera is decisive and it **fails open** (only a confident warm-lit-interior `.notSky` blocks; GPS accuracy is logged but never blocks alone — a code-review hardening so degraded urban GPS can't false-block a real catch). Wired into the catch path **shadow-mode first** (`outdoor_gate_shadow`, never blocks), with flag-gated enforcement (`catch_blocked_outdoors` + a friendly "head outside" nudge) defaulting **OFF** until the field gate passes. A debug-overlay "Sky gate" row flips shadow↔enforce on-device. Offline validator + protocol at `tools/authenticity-gate/`.
