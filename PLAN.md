...
| 3 | Visual confirmation go-live (Vision + COCO `airplane`) | Bet A · Real-catch | ~1–2d | ML half of accuracy (compass wobble); 3D-pinhole geometry shipped v0.2.1. Scaffolded but **dormant in Release** (`#if DEBUG`). Validate vs pin-protocol ground truth in `Documents/replays`, then enable. Mechanism for the outdoor/in-frame authenticity gate — decide alongside. Wishlist #7. |
...
- **Remaining (Noah / on-device):** U9 — run the visual-confirmation field gate (≥2× median screen-error reduction) **and** the authenticity-gate field validation (`tools/authenticity-gate/FIELD-TEST.md`), then flip the respective Release defaults; U10 follow-ups — bundle the Apache-2.0 YOLOX `LICENSE`/NOTICE + add YOLOX to the Attributions page (model is stock YOLOX-S COCO, Apache-2.0 — see `tools/visual-confirmation/MODEL-LICENSE.md`); **deferred** — multi-catch confirm/deny, and snapping the saved catch-photo bracket to the detected bbox. See CHANGELOG 2026-06-24.
