...
**TestFlight v0 prep landed 2026-05-26.** Everything required to build, sign, and upload a TestFlight build is in place. Highlights: credential baking via xcconfig + Info.plist (`Tailspot.xcconfig` + gitignored `Tailspot.secrets.xcconfig` → Info.plist `OpenSkyClientID/Secret` keys → `Bundle.main.infoDictionary` at runtime, with env-var fallback preserving the dev iteration loop); manual `Info.plist` replacing the auto-generated one (adds `ITSAppUsesNonExemptEncryption=false` and the custom OpenSky keys); `PrivacyInfo.xcprivacy` declaring UserDefaults + FileTimestamp required-reason APIs and the precise-location + photos data categories; `MARKETING_VERSION=0.1.0` + `CURRENT_PROJECT_VERSION=1`; programmatically generated 1024×1024 app icon (3 luminosity variants via `tools/generate-app-icon.swift`). 169 unit tests still pass. **Apple-side steps** (App Store Connect record, signing in Xcode, Archive + Upload, TestFlight beta tester invite) live in `docs/testflight-handoff.md` — Claude can't do these, they require Noah in Apple UIs. **Security accepted for v0:** the OpenSky secret is in the shipped binary, extractable from any .ipa; rotation is the mitigation if a tester device is compromised. Backend proxy (Pending #2) is the path for wider distribution.
