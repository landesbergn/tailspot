...
└─ ios/                     ← Xcode project
   └─ Tailspot/
      ...
      │  └─ ReplayAnalyzer.swift    — offline replay through annotation + visibility + lock-on
      ├─ TailspotTests/
...
Friday POC (§3.0a) shipped 2026-05-07. The deploy loop (§3.0c) shipped 2026-05-13. Through 2026-05-17, also delivered: aircraft type lookup, lock-on interaction, catch flow v0, clean default UI + debug toggle, 30 km visibility cap, heading-accuracy color cue, partial device log streaming, **Hangar (collection) v0**, **Replay recorder v0**, **Replay analyzer v0**.

**Pending (priority order):**

| # | Item | Est. | Why |
|---|------|------|-----|
| 1 | **Camera zoom (pinch + buttons)** | ~1 hr | Distant cruise traffic at 20+ km is a few pixels wide at 1×. Pinch-to-zoom on the camera preview (mapped to `AVCaptureDevice.videoZoomFactor`) plus optional 0.5×/1×/2× buttons makes them spottable. Need to: pipe the zoom factor into `CameraPreview`'s `AVCaptureSession`, AND scale the projection math accordingly (zoom narrows effective FOV — `hfovDeg / zoom`, same for vfov), so the lock brackets stay attached to the actual plane through the zoom. |
| 2 | **Tap-to-lock on an arbitrary plane** | ~1–2 hr | Today the lock-on target is whichever visible plane sits closest to screen center (within 80 px). With zoom, the user wants to point + tap a specific plane to ID it — closest-to-tap-point, not closest-to-center. Likely an explicit "tap target" mode in `LockOnEngine` that overrides the center-driven heuristic for ~stickyHoldDuration (or until tap-elsewhere clears). Has to coexist with the current acquire→lock animation. |
...
| 4 | **Replay analyzer summary + load UI** | ~1–2 hr | Analyzer ships structured `ReplayReport`s; the missing pieces are a `describe()` human-readable formatter and a way to feed a `.jsonl` in without writing Swift (CLI binary, or an in-app debug "Load replay" picker). Becomes interesting the moment Noah records a real session. |
| 5 | **Hangar v1 polish** | ~2–3 hr | Things v0 deliberately left out: dedupe (collapse repeat catches of the same icao24 under a single row with "×N" badge), swipe-to-delete with confirm alert, illustrated-card stub for top type×airline combos. Defer until there's real catch volume to design against. |
| 6 | **Planespotters photo API integration** | ~half day, plus T&Cs review | Pull real aircraft photos by registration / hex from `https://www.planespotters.net/photo/api`. Would replace the commissioned-illustrated-cards plan from §1.4 (cheaper, ships now, no licensing exposure if their TOS allow non-commercial use). Read their API docs + TOS before wiring; cache per-icao24 alongside `MetadataCache`. Surfaces in `CatchDetailView` and the locked-label flow first; Hangar v1 dedupe card art second. Updates §6.3 (photo strategy default) if it works out. |
| 7 | **Visual confirmation (Vision + COCO airplane class)** | ~1 day | Per §1.1a. Detect the actual plane image in the camera frame and lock the brackets to it rather than the compass-predicted position. Hardest of these; defer until #4 lands so accuracy improvements can be measured against recorded sessions. |
| 8 | **Achievements / streaks / scoring** | open | Phase 2 work. Wait until Hangar has real catch volume to design against. |
| 9 | **Rotate leaked OpenSky client secret** | ~10 min | Long-standing security debt from commit `869d06d`. Demoted to the bottom 2026-05-17 per Noah: anonymous-tier remaining credit headroom and field-test cadence make this not-urgent for now. Still worth doing eventually (10 min, regenerate on opensky-network.org and update the user-only scheme's env var) — but not blocking anything. |
