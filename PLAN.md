...
- **Visual confirmation (U8):** user-facing Settings → CAMERA "Snap reticle to plane" beta toggle (default off); the pipeline reads its enable key live. The pipeline itself was already built and live in DEBUG — go-live is the toggle + the field gate, not a build.
