...
| 3 | **Visual confirmation (Vision + COCO airplane class)** | ~1 day | Per §1.1a. Detect the actual plane image in the camera frame and lock the brackets to it rather than the compass-predicted position. Now the top accuracy lever: the 2026-06-06 field round quantified the residual horizontal offset as compass wobble (±20° actual urban vs ±10° claimed; one 165°-in-1s jump on record) plus ~1/cos(camElev) horizontal exaggeration in the separable projection at high pitch. A cheaper partial step is proper 3D rotation projection in `Geo.screenPosition` (fixes the systematic ~25%-at-40° part; compass wobble remains). Measure any attempt against the pin-protocol recordings in `Documents/replays` — they carry tap-confirmed ground truth. |
