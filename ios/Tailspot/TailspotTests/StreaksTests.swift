...
struct StreaksTests {
    ...
    @Test func daySetUnionsExtraDays() {
        let row = mk(caughtAt: Self.aug17midnightUTC)
        row.caughtDayKey = "2026-08-17"
        let days = Streaks.daySet(catches: [row], extraDays: ["2026-08-16"])
        #expect(days == ["2026-08-16", "2026-08-17"])
    }
    ...
    @Test func ledgerRecordsEachDayOnce() {
        let d = freshDefaults("streak-ledger-once")
        let ledger = StreakLedger(defaults: d)
        #expect(ledger.record("2026-08-17"))          // first action of the day
        #expect(!ledger.record("2026-08-17"))         // later same-day actions: no edge
        #expect(ledger.record("2026-08-18"))
        #expect(ledger.days() == ["2026-08-17", "2026-08-18"])
    }

    @Test func ledgerPrunesOldestBeyondTheCap() {
        let d = freshDefaults("streak-ledger-prune")
        let ledger = StreakLedger(defaults: d)
        var key = "2020-01-01"
        for _ in 0..<(StreakLedger.maxEntries + 5) {
            ledger.record(key)
            key = Streaks.key(byAdding: 1, to: key)!
        }
        let days = ledger.days()
        #expect(days.count == StreakLedger.maxEntries)
        #expect(!days.contains("2020-01-01"))   // oldest pruned
        // The most recent entry survives.
        #expect(days.contains(Streaks.key(byAdding: -1, to: key)!))
    }
