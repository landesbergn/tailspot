...
export type Theme = {
  ...
  accent: string;
  muted: string;
  /// Optional CSS gradient for non-inverted slide backgrounds; falls back
  /// to a subtle bg-derived gradient when absent.
  bgGradient?: string;
  /// Optional decorative motif for non-inverted slides ("aviation" swaps
  /// the accent blobs for distant twin contrails).
  decor?: "aviation";
};
